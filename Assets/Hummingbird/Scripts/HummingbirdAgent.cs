...
public class HummingbirdAgent : Agent
{
    ...
    /// <summary>
    /// Resetst he agent when an episode begins
    /// </summary>
    public override void OnEpisodeBegin()
    {
        if (trainingMode)
        {
            // only resets flowers in training when there is one agent per area
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        // Calculate pitch and yaw rotations from our input
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // If the nearestFlower is null, observe an empty array and return early
        ...
        // observe the agent's local rotation (a quaternion so it's 4 observations)
        sensor.AddObservation(transform.localRotation.normalized);

        // Get a vector from the beak tip to the nearest flower
        Vector3 toFlower = nearestFlower.FlowerCenterPosition - beakTip.position;

        // Observe a normalized vector pointing to the nearest flowr (vector3 so 3 observations)
    ...
    /// <summary>
    ///  When behaviour Type is set to Heuristic Only on the agent's Behavior Parameters this function will be called
    ...
    private void TriggerEnterOrStay(Collider collider)
    {
        ...
        if (collider.CompareTag("nectar"))
        {
            ...
            // Check if the closest collising point is close to the beak tip
            ...
            if(Vector3.Distance(beakTip.position, closestPointToBeakTip) < BeakTipRadius)
            {
                ...
                // Note: this is per fixed timestap, meaning it happens every .02 seconds, or 50X per second
    ...
    /// <summary>
    /// Called when an agent collides with something solid
    /// </summary>
    ...
    private void MoveToSafeRandomPosition(bool inFrontOfFlower)
    {
        ...
        {
            attemptsRemaining--;
            if(inFrontOfFlower && flowerArea.Flowers.Count > 0)
            {
            ...
            else
            {
                ...
                // pick a random radius from the center of the area (the tree)
    ...
    private void UpdateNearestFlower()
    {
        foreach (Flower flower in flowerArea.Flowers)
        {
            ...
            {
                // already have a nearest flower and the current flower does have nectar
                ...
                if(distanceToFlower < distanceToCurrentNearestFlower || !nearestFlower.HasNectar )
                {
                    nearestFlower = flower;
                }
